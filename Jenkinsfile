    ...
    stages {
        ...
        stage('Deploy Monitoring Config') {
            steps {
                ...
                sh """docker run --network host --user root --rm -v ${env.WORKSPACE}:/work -w /work -v ${KUBECONFIG_PATH}:/tmp/config -e KUBECONFIG=/tmp/config bitnami/kubectl:latest apply -k infra/k8s/monitoring/prometheus"""
        ...
        stage('Deploy Monitoring') {
            steps {
                ...
                sh """
                    ...
                    docker run --network host --user root --rm -v ${env.WORKSPACE}:/work -w /work -v ${KUBECONFIG_PATH}:/tmp/config -e KUBECONFIG=/tmp/config bitnami/kubectl:latest apply -k infra/k8s/monitoring/prometheus
